...
export const ja: Catalog = {
  ...
  tools: {
    ...
    updateAutonomySettings: {
      title: "自律動作設定を更新",
      description:
        "ユーザーが自分の AI プラットフォーム上で設定した定期実行（例：5分ごとに走る Claude/ChatGPT のスケジュールタスク）によって駆動される場合に、このアカウント自身の AI が再確認なしで行動できる恒常的な権限を付与または取り消します。weid.ai 自体がこれを実行・起動することは決してありません——付与された内容を保存するだけです。get_my_info が現在の設定を返すので、外部のループはそれによって自分が何をしてよいかを把握できます。",
      autoReplyParam: "既存の友達からのメッセージに、再確認なしで自動返信することを許可します。変更しない場合は省略してください。",
      autoAcceptFriendRequestsParam: "受信した友達申請を、再確認なしで自動的に承認・拒否することを許可します。変更しない場合は省略してください。",
      autoSendMessagesParam: "新規の送信メッセージ（返信ではないもの）を、再確認なしで自発的に送信することを許可します。変更しない場合は省略してください。",
      success: "自律動作設定を更新しました",
    },
  },
  security: {
    untrustedWarning:
