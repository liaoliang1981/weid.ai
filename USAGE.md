...
1. Add the custom connector `https://mcp.weid.ai` in Claude, ChatGPT, Manus, or Grok (see [weid.ai](https://weid.ai) for the exact menu path on each).
...
1. 在 Claude、ChatGPT、Manus 或 Grok 里添加自定义连接器 `https://mcp.weid.ai`（各平台具体路径见 [weid.ai](https://weid.ai) 首页）。
...
1. Claude、ChatGPT、Manus、または Grok でカスタムコネクタ `https://mcp.weid.ai` を追加してください（各プラットフォームでの具体的なメニューの場所は [weid.ai](https://weid.ai) を参照）。
...
1. Claude, ChatGPT, Manus, 또는 Grok에서 커스텀 커넥터 `https://mcp.weid.ai`를 추가하세요 (각 플랫폼의 정확한 메뉴 경로는 [weid.ai](https://weid.ai)를 참고하세요).
...
1. Agrega el conector personalizado `https://mcp.weid.ai` en Claude, ChatGPT, Manus o Grok (consulta [weid.ai](https://weid.ai) para ver la ruta exacta del menú en cada uno).
...
1. Ajoutez le connecteur personnalisé `https://mcp.weid.ai` dans Claude, ChatGPT, Manus ou Grok (voir [weid.ai](https://weid.ai) pour le chemin exact du menu sur chacun).
...
1. Fügen Sie den benutzerdefinierten Connector `https://mcp.weid.ai` in Claude, ChatGPT, Manus oder Grok hinzu (den genauen Menüpfad für jede Plattform finden Sie auf [weid.ai](https://weid.ai)).
...
1. Adicione o conector personalizado `https://mcp.weid.ai` no Claude, ChatGPT, Manus ou Grok (veja [weid.ai](https://weid.ai) para o caminho exato do menu em cada um).
...
1. เพิ่มคอนเนกเตอร์แบบกำหนดเอง `https://mcp.weid.ai` ใน Claude, ChatGPT, Manus หรือ Grok (ดูเส้นทางเมนูที่แน่นอนของแต่ละแพลตฟอร์มได้ที่ [weid.ai](https://weid.ai))
