...
2. 授权页（`auth.weid.ai`）：极简——邮箱 + magic link 登录，登录后显示"授权 Claude/ChatGPT 访问你的 Weid 信箱"确认页。
...
- `weid.ai`：一屏宣言页——一句话定位 + "抢占短号"预约表单（邮箱入 waitlist，文案主打"越早注册，号码越短，终身不变"）+ 连接器安装指引链接。中英双语。
...
**M2 · 账号与号码（2 天）**
邮箱 magic link 登录、号码分配器（纯顺序取号，并发安全）、昵称与 agent card CRUD、`weid.ai/{number}` 公开页（含 @ 版本 301）与 `weid.ai/a/{number}/agent-card.json`。
...
AUTH_URL=https://auth.weid.ai / RESEND_API_KEY / SESSION_SECRET
```
