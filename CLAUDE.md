...
6. **身份认证不用邮箱，用"号码 + 密码"**：注册时只需设一个密码，系统立刻分配号码；号码本身设计上就是公开可分享的（"加我好友 @10024"），不能兼职当登录凭证，所以另用一个自己设的密码承担"证明这是你账号"的职责，登录方式是"Weid 号 + 密码"，跟号码本身分开——类似传统网站的用户名+密码，只是用户名换成了系统分配的号码。密码只存 scrypt 哈希，忘记密码 = 无法找回该账号，没有邮箱找回等备用通道。相应地，**本阶段不做邮件通知**（新好友申请/新消息不再发邮件提醒，需要用户主动让 AI 调用 `check_inbox` 查看）。
...
```
users:        id, password_hash            -- scrypt(salt:hash)，登录时按 accounts.number 查到 user 再校验
...
2. 授权页（`auth.weid.ai`）：无会话时先给一个选择——"注册新号"（设一个密码即分配号码）或"已有号，登录"（输入 Weid 号 + 密码）；登录后显示"授权 Claude/ChatGPT 访问你的 Weid 信箱"确认页。
...
- `auth.weid.ai`：注册/登录（号码+密码）/授权/领号页面。
...
6. 密码只存 scrypt 哈希，明文密码不落日志、不落库；登录只能靠"号码+密码"本身，系统不提供任何"忘记密码"的旁路（否则等于给攻击者留后门）。
...
**M2 · 账号与号码（2 天）**
号码+密码注册登录、号码分配器（纯顺序取号，并发安全）、昵称与 agent card CRUD、`weid.ai/{number}` 公开页（含 @ 版本 301）与 `weid.ai/a/{number}/agent-card.json`。
