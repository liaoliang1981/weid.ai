...
构建一个 **MCP 远程服务器 + 轻量后端**，让任何 claude.ai 或 ChatGPT 用户通过添加自定义 Connector 获得一个 2088 号码（系统分配，5 位起）+ 自定义昵称和一个收件箱；用户之间**先加好友、后通信**（陌生人只能发好友申请），从而实现：Claude 用户与 ChatGPT 用户的 AI 之间互相发送结构化消息。**本阶段不做**：agent 自主托管、7×24 自动应答、A2A 对外联邦、支付、实体验证。先让信箱跑通。

**成功画面**：用户 A 在 claude.ai 对 Claude 说"向 @10024 发好友申请，说明我想询胶原蛋白肽"；用户 B 在 ChatGPT 里说"查我的 2088"，B 的 GPT 读出申请、B 说"通过"；随后 A 发出询盘，B 的 GPT 读出消息并起草回复。全程两人不离开各自的 AI。
...
关键设计决定（已定，勿改）：
1. **用户身份 = 号码 + 昵称（QQ 模式）**。号码由系统顺序分配、终身不变、全网唯一、**不可转让**，从 10000 起（5 位），号段用尽自然进位到 6 位——**号码长度即资历**。昵称用户自定义、可随时改、不要求唯一、支持任意语言（含中文）。对话中写作 `@10024`，口语传播为"我的 2088 号是 10024"。`{number}@2088.ai` 是仅用于协议层/跨网络路由的机器格式，永不在产品界面中作为身份展示。
...
2. `register_account(nickname)` — 为当前用户分配号码（系统顺序取号，用户不可选号）并设置昵称。每用户限 1 个免费号码。返回文案须突出号码："你的 2088 号是 @10024，越早注册号越短，此号终身归你。"
...
11. `send_message(to_number, subject, body_text, structured?)` — 发送消息，一步发出。`to_number` 接受 `@10024`、`10024`、`10024@2088.ai` 三种写法并归一化。**好友门槛硬规则**：双方非好友时拒绝发送，返回错误"对方还不是你的好友，请先用 send_friend_request 发好友申请"。对方号码不存在或 suspended 同样明确报错。
...
✅ 验收：连续注册 3 个账号获得 10000、10001、10002；浏览器访问 `2088.ai/10000` 看到名片、`2088.ai/@10000` 正确跳转；并发 50 请求注册无重号。
