...
构建一个 **MCP 远程服务器 + 轻量后端**，让任何 claude.ai 或 ChatGPT 用户通过添加自定义 Connector 获得一个 Weid 号码（系统分配，从 10000 起）+ 自定义昵称和一个收件箱；用户之间**先加好友、后通信**（陌生人只能发好友申请），从而实现：Claude 用户与 ChatGPT 用户的 AI 之间互相发送结构化消息。**本阶段不做**：agent 自主托管、7×24 自动应答、A2A 对外联邦、支付、实体验证。先让信箱跑通。

**成功画面**：用户 A 在 claude.ai 对 Claude 说"向 @10024 发好友申请，说明我想询胶原蛋白肽"；用户 B 在 ChatGPT 里说"查我的 Weid 号"，B 的 GPT 读出申请、B 说"通过"；随后 A 发出询盘，B 的 GPT 读出消息并起草回复。全程两人不离开各自的 AI。
...
关键设计决定（已定，勿改）：
1. **用户身份 = 号码 + 昵称（QQ 模式）**。号码由系统顺序分配、终身不变、全网唯一、**不可转让**，从 10000 起（5 位），号段用尽自然进位到 6 位——**号码长度即资历**。昵称用户自定义、可随时改、不要求唯一、支持任意语言（含中文）。对话中写作 `@10024`，口语传播为"我的 Weid 号是 10024"。`{number}@weid.ai` 是仅用于协议层/跨网络路由的机器格式，永不在产品界面中作为身份展示。
...
6. **身份认证不用邮箱，用"匿名注册 + 恢复码"**：注册不需要填任何东西，系统立刻分配号码并生成一个高熵恢复码（仅显示一次）；号码本身设计上就是公开可分享的（"加我好友 @10024"），不能兼职当登录凭证，所以另用一个私密的恢复码承担"证明这是你账号"的职责。换设备或重新走 OAuth 授权时输入恢复码即可找回同一个号码；恢复码遗失 = 无法找回该账号，没有邮箱找回等备用通道。相应地，**本阶段不做邮件通知**（新好友申请/新消息不再发邮件提醒，需要用户主动让 AI 调用 `check_inbox` 查看）。
...
```
users:        id, recovery_code_hash(unique) -- 唯一凭证，只在生成时明文出现过一次
...
2. `register_account(nickname)` — 为当前用户分配号码（系统顺序取号，用户不可选号）并设置昵称。每用户限 1 个免费号码。返回文案须突出号码："你的 Weid 号是 @10024，越早注册号越短，此号终身归你。"
...
11. `send_message(to_number, subject, body_text, structured?)` — 发送消息，一步发出。`to_number` 接受 `@10024`、`10024`、`10024@weid.ai` 三种写法并归一化。**好友门槛硬规则**：双方非好友时拒绝发送，返回错误"对方还不是你的好友，请先用 send_friend_request 发好友申请"。对方号码不存在或 suspended 同样明确报错。
12. `search_directory(query, limit?=10)` — 在公开名片中按昵称/能力/描述全文检索，返回号码+昵称列表。这是"电话簿"，也是昵称不唯一时找人的正道。

工具实现共同要求：
- 所有工具须先通过 OAuth token 解析出 user → number，未注册号码的用户调用 4–12 时返回引导性错误："你还没有 Weid 号，请先用 register_account 注册"。
...
2. 授权页（`auth.weid.ai`）：无会话时先给一个选择——"注册新号"（点一下即分配号码+生成恢复码，页面明文显示恢复码且仅此一次，提示用户务必保存）或"已有恢复码登录"（输入恢复码）；登录后显示"授权 Claude/ChatGPT 访问你的 Weid 信箱"确认页。
...
6. 恢复码只存哈希，明文只在生成的那一次响应里出现，之后任何日志、错误信息、数据库记录都不得再现明文；找回账号只能靠恢复码本身，系统不提供任何"忘记恢复码"的旁路（否则等于给攻击者留后门）。
...
**M2 · 账号与号码（2 天）**
匿名注册 + 恢复码登录、号码分配器（纯顺序取号，并发安全）、昵称与 agent card CRUD、`weid.ai/{number}` 公开页（含 @ 版本 301）与 `weid.ai/a/{number}/agent-card.json`。
